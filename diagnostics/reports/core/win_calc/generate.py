#!/usr/bin/env python3
"""
Win Calc report generators

- Summary adjustments report (short table)
- Detailed calculation report (per-player breakdown)
- Missing windows report (players lacking rolling windows data)

Outputs default to diagnostics/reports/output/win_calc with short-date filenames.
"""
...
# Outputs
DEFAULT_OUT_ROOT = Path("/mnt/storage_fast/workspaces/red_ocean/diagnostics/reports/output/win_calc")

# Adjustment parameters (mirror src/win_calc/rolling_adjuster.py)
...
def missing_windows_sections() -> List[str]:
    lines: List[str] = []
    for site in ("fanduel", "draftkings"):
        latest = find_latest_slate(site)
        if latest is None:
            lines.append(f"### {site.title()} — No data found")
            continue

        # Load once
        bat_all = collect_records(site, latest, "batter")
        pit_all = collect_records(site, latest, "pitcher")
        # Augment pitchers with any records whose position looks like pitcher
        def is_pitcher_pos(pos: str | None) -> bool:
            if not pos:
                return False
            up = pos.upper()
            return up in ("P", "SP", "RP") or ("PITCH" in up)
        extra_pit = [r for r in bat_all if is_pitcher_pos(r.position)]
        if extra_pit:
            # Merge uniquely by (name, team)
            seen = {(r.name, r.team) for r in pit_all}
            for r in extra_pit:
                key = (r.name, r.team)
                if key not in seen:
                    pit_all.append(r)
                    seen.add(key)
        bat_starters = [r for r in bat_all if r.is_starter]
        pit_starters = [r for r in pit_all if r.is_starter]

        def compute_missing(recs: List[Record], role: str) -> List[Tuple[Record, str]]:
            missing: List[Tuple[Record, str]] = []
            rosters = _load_active_rosters()
            index = _build_name_index(rosters)
            for r in recs:
                norm_name = _normalize_name(r.name or "")
                mlb_id = index.get((norm_name, _normalize_team_abbr((r.team or "").upper())))
                if not mlb_id:
                    any_team = [pid for (n, _t), pid in index.items() if n == norm_name]
                    if any_team:
                        mlb_id = any_team[0]
                    else:
                        try:
                            ...
                            for person in people:
                                ...
                                if _normalize_name(full) != norm_name:
                                    continue
                                team = (person.get("currentTeam") or {}).get("abbreviation")
                                if team and _normalize_team_abbr(str(team)) == _normalize_team_abbr((r.team or "").upper()):
                            ...
                            pass
                        if not mlb_id:
                            missing.append((r, "no_mlb_id"))
                            continue
                rolling = _load_rolling_file(mlb_id, role)
                if not rolling:
                    missing.append((r, "no_rolling_file"))
                    continue
                x50 = _latest_xwoba(rolling, "50")
                x100 = _latest_xwoba(rolling, "100")
                x250 = _latest_xwoba(rolling, "250")
                miss_keys = [k for k, v in (("50", x50), ("100", x100), ("250", x250)) if v is None]
                if miss_keys:
                    missing.append((r, ",".join(miss_keys)))
            return missing

        # Compute all four categories
        sections = [
            ("Missing Starting Pitchers", compute_missing(pit_starters, "pitcher")),
            ("Missing Pitchers (All Pool)", compute_missing(pit_all, "pitcher")),
            ("Missing Starting Batters", compute_missing(bat_starters, "batter")),
            ("Missing Batters (All Pool)", compute_missing(bat_all, "batter")),
        ]

        for title_suffix, missing in sections:
            lines.append(f"### {site.title()} — {title_suffix}")
            lines.append("")
            lines.append(f"- **count**: {len(missing)}")
            lines.append("")
            lines.append("| Name | Team | Missing |")
            lines.append("|---|---|---|")
            for r, why in missing:
                lines.append(f"| {fmt_short_name(r.name)} | {r.team or '?'} | {why} |")
            lines.append("")
    return lines
...
def _build_missing_content() -> List[str]:
    content: List[str] = []
    content.append("## Win Calc Missing Rolling Windows")
    content.append("")
    content.append(f"Generated: {datetime.now().isoformat(timespec='seconds')}")
    content.append("")
    content.extend(missing_windows_sections())
    return content


def generate_reports(out_root: Path | None = None, out_file: Path | None = None, split: bool = True, include_missing: bool = True) -> Tuple[Path, Path | None, Path | None]:
    if out_root is None:
    ...
    if out_file is not None:
        ...
        missing_path = out_file.with_name(out_file.stem.replace("_adj", "") + "_missing_windows.md") if include_missing else None
    else:
        summary_path = out_root / f"{short_date}_adj.md"
        calc_path = (out_root / f"{short_date}_adj_calc.md") if split else None
        missing_path = (out_root / f"{short_date}_missing_windows.md") if include_missing else None
    ...
    # Write missing windows
    if include_missing and missing_path is not None:
        missing_path.parent.mkdir(parents=True, exist_ok=True)
        with open(missing_path, "w") as f:
            f.write("\n".join(_build_missing_content()))

    return summary_path, calc_path, missing_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate win_calc reports (summary, calc, missing)")
    parser.add_argument("--out-dir", dest="out_dir", default=str(DEFAULT_OUT_ROOT), help="Output directory for reports")
    parser.add_argument("--out-file", dest="out_file", default=None, help="Optional explicit summary file path; calc will be *_calc.md and missing *_missing_windows.md next to it")
    parser.add_argument("--no-split", dest="split", action="store_false", help="Write only the summary report (no separate calc report)")
    parser.add_argument("--no-missing", dest="missing", action="store_false", help="Do not write the missing windows report")
    args = parser.parse_args()

    summary_path, calc_path, missing_path = generate_reports(
        Path(args.out_dir),
        Path(args.out_file) if args.out_file else None,
        split=bool(args.split),
        include_missing=bool(args.missing),
    )
